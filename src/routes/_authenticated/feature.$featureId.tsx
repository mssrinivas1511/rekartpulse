...
export const Route = createFileRoute("/_authenticated/feature/$featureId")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Feature adoption, client usage, feedback and product notes.",
      },
  ...
  notFoundComponent: () => (
    <div className="p-8">
      <h1 className="font-semibold">Feature not found</h1>
      <Link to="/features" className="mt-3 inline-block text-sm text-primary">
        Back to features
      </Link>
    </div>
  ),
...
function FeatureDetailPage() {
  ...
  const { data } = useSuspenseQuery({
    ...
    queryFn: () => getFeatureDetail({ data: { id: featureId } }),
  });
  ...
  async function remove() {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Could not delete feature");
    }
  }
  ...
  return (
    <div className="space-y-5 p-6">
      ...
      >
        <ArrowLeft className="size-3.5" /> Back to features
      </Link>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {f.category} · {f.release_version ?? "No release version"}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          {can("features", "delete") && (
            <ConfirmDialog
              ...
              description="This removes the feature and its related records."
              onConfirm={remove}
              trigger={
                <Button variant="outline" size="sm">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          ["Clients using", `${f.clients_using}/${f.clients_enabled}`],
          ["Customers reached", f.customers_reached.toLocaleString("en-IN")],
          ["Feedback", f.feedback_count],
        ].map(([label, value]) => (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-lg border bg-card p-4">
          <h2 className="mb-3 text-sm font-semibold">Client adoption</h2>
          <div className="space-y-2">
            {data.client_features.map((cf) => (
              <div
                key={cf.id}
                className="flex items-center justify-between border-b py-2 text-sm last:border-0"
              >
                <span>{cf.client_name}</span>
                <span className="text-muted-foreground">
                  {cf.enabled ? `${cf.adoption_percent}%` : "Not enabled"}
                </span>
              </div>
            ))}
            {data.client_features.length === 0 && (
              <p className="text-sm text-muted-foreground">No client adoption recorded.</p>
            )}
          </div>
        </section>
        <section className="rounded-lg border bg-card p-4">
        ...
        <section className="rounded-lg border bg-card p-4 lg:col-span-2">
          <h2 className="mb-3 text-sm font-semibold">Product notes</h2>
          <div className="grid gap-3 md:grid-cols-3">
            {data.notes.map((note) => (
              <div key={note.id} className="rounded-md bg-muted p-3">
                <StatusBadge status={note.note_type} />
                <p className="mt-2 text-sm">{note.content}</p>
                <p className="mt-2 text-xs text-muted-foreground">{note.created_by_name}</p>
              </div>
            ))}
            {data.notes.length === 0 && (
