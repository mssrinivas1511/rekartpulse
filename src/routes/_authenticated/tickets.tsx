...
function TicketsPage() {
  ...
  return <div className="space-y-4"><PageHeader title="Tickets" description={`${data.length} client requests and issues`} />
    <div className="flex flex-wrap gap-1.5 px-6">{FILTERS.map((item) => <button key={item} onClick={() => setStatus(item)} className={status === item ? "rounded-full bg-primary px-3 py-1 text-xs text-primary-foreground" : "rounded-full border px-3 py-1 text-xs text-muted-foreground hover:bg-accent"}>{item.replace("_", " ")}</button>)}</div>
